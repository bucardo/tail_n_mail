...
for my $file (sort keys %fileslurp) {
	for my $linenum (sort {$a <=> $b} keys %{$fileslurp{$file}}) {
		for my $func (sort keys %{$fileslurp{$file}{$linenum}}) {
			...
			## Exception for "or fail"
			if ($fileslurp{$file}{$linenum}{$func} =~ / or fail/) {
				pass $t;
				next;
			}
...
for my $filename (qw{ tail_n_mail }) {
...
for my $filename (sort @testfiles) {
  ...
  VIO: for my $v (@vio) {
		...
		## We do not want to version our test files
		next if $policy =~ /RequireVersionVar/;
